...
export const ProfileScreen = ({ customerId, onBack }: ProfileScreenProps) => {
  ...
  const StatCard = ({ icon, label, value, color, iconColor }: any) => (
    <View className="bg-slate-50/60 p-4 rounded-[24px] border border-slate-100 w-[48%] mb-4 flex-row items-center">
      <View style={{ backgroundColor: color }} className="w-10 h-10 rounded-xl items-center justify-center mr-3">
        <Ionicons name={icon} size={18} color={iconColor} />
      </View>
      <View className="flex-1">
        <Text className="text-slate-900 text-xs font-black mb-0.5" numberOfLines={1}>{value || "N/A"}</Text>
        ...
        <View className="flex-row justify-between items-start mb-6">
          <View className="flex-1 pr-4">
            <Text className="text-slate-900 text-2xl font-black tracking-tight mb-1">
              {customer.borrowerName || customer.applicantName || 'Applicant'}
            </Text>
            <View className="flex-row items-center">
              <Ionicons name="call-outline" size={12} color="#94A3B8" />
              <Text className="text-slate-400 text-xs font-bold ml-1">{customer.mobile || customer.mobileNumber}</Text>
            </View>
          </View>
          {(customer.isPanVerified || customer.borrowerName) && (
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
          ...
          <View className="flex-row flex-wrap justify-between mt-2 mb-4">
            ...
            <StatCard 
              icon="briefcase" 
              label="Employment" 
              value={customer.employmentType ? customer.employmentType.replace('_', ' ') : undefined} 
              color="#FFF7ED" 
              iconColor="#F97316" 
            />
            <StatCard 
              ...
              value={customer.monthlyIncome ? formatCurrency(customer.monthlyIncome) : undefined} 
              color="#F5F3FF" 
              iconColor="#8B5CF6" 
            />
            <StatCard 
          ...
          {/* Addresses Card */}
          <View className="mb-8">
            <Text className="text-slate-900 text-lg font-black mb-4">Addresses</Text>
            
            {/* Home Address */}
            <View className="bg-slate-50 p-5 rounded-[24px] border border-slate-100 mb-4">
              <View className="flex-row items-start">
                ...
                <View className="flex-1">
                  <Text className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Residential Address</Text>
                  <Text className="text-slate-900 font-bold text-sm leading-5">
                    {customer.address ? (
                      `${customer.address.house || ''} ${customer.address.building || ''}, ${customer.address.area || ''}, ${customer.address.city || ''}, ${customer.address.state || ''} - ${customer.address.pincode || ''}`.trim().replace(/,\s*,/g, ',').replace(/^,\s*|\s*,\s*$/g, '')
            ...
            {customer.officeAddress && (
              <View className="bg-slate-50 p-5 rounded-[24px] border border-slate-100">
                <View className="flex-row items-start">
                  ...
                  <View className="flex-1">
                    <Text className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Office Address</Text>
                    {customer.officeAddress.officeName && (
                      <Text className="text-slate-950 font-black text-sm mb-1">{customer.officeAddress.officeName}</Text>
                    )}
                    <Text className="text-slate-900 font-bold text-sm leading-5">
                      {`${customer.officeAddress.building || ''}, ${customer.officeAddress.area || ''}, ${customer.officeAddress.city || ''}, ${customer.officeAddress.state || ''} - ${customer.officeAddress.pincode || ''}`.trim().replace(/,\s*,/g, ',').replace(/^,\s*|\s*,\s*$/g, '')}
                    </Text>
                  </View>
                </View>
              </View>
          ...
          {customer.customerBanks && customer.customerBanks.length > 0 ? (
            customer.customerBanks.slice(0, 1).map((bank: any) => (
              <LinearGradient
                key={bank.id || bank.accountNo}
            ...
            <View className="bg-slate-50 p-6 rounded-[32px] border border-slate-100 mb-8 items-center justify-center">
              <MaterialCommunityIcons name="bank-off" size={30} color="#94A3B8" />
              <Text className="text-slate-400 text-xs font-bold mt-2">No bank account linked</Text>
            </View>
          )}
